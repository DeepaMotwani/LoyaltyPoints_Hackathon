...
customer_name, err := stub.GetState(customerNamePrint)
...
var key string
	...
	if len(args) != 1 {
        ...
        key=args[0]
    customer_name, err := stub.GetState(key)
          if err != nil {
		...
		customer_name_temp, err := stub.GetState(string(customer_name))

		 loyalty_points, err := stub.GetState(string(customer_name_temp))

		// if err != nil {
		//jsonResp = "{\"Error\":\"Failed to get state for " + key + "\"}"
		//return nil, errors.New("Error starting Simple chaincode")
	//}
